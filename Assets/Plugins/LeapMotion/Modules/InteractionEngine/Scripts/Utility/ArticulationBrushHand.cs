    ...
    public class ArticulationBrushHand : HandModel
    {
        ...
        public override void UpdateHand()
        {
            ...
            for (int fingerIndex = 0; fingerIndex < N_FINGERS; fingerIndex++)
            {
                if (loPolyHandPalm != null)
                {
                    //curJoint = GetJointTransform(fingerIndex);
        ...
        private Transform GetJointTransform(int fingerIndex)
        {
            if (fingersTransform != null && fingersTransform.Length > 0)
                return fingersTransform[fingerIndex];

            else if (loPolyHandPalm != null)
            {
                if (fingerIndex == 0)
                    return loPolyHandPalm.GetChild(fingerIndex);
                return loPolyHandPalm.GetChild(fingerIndex).GetChild(0);
            }
            return null;
        }
