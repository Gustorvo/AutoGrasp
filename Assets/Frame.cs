namespace SoftHand
{
    ...
    {
        [ThreadStatic]
        private static Queue<ArticulatedHand> _handPool;
        ...
        public Frame()
        {
            Hands = new List<ArticulatedHand>();
        }

        /// <summary>
        /// Constructs a new Frame.    
        /// </summary>
        public Frame(long id, long timestamp, float fps, List<ArticulatedHand> hands)
        {
            Id = id;
            Timestamp = timestamp;
        ...
        /// </summary>
        public ArticulatedHand Hand(int id)
        {
        ...
        /// </summary>
        public List<ArticulatedHand> Hands;
        ...
        internal void ResizeHandList(int count)
        {
            if (_handPool == null)
            {
                _handPool = new Queue<ArticulatedHand>();
            }

            while (Hands.Count < count)
            {
                ArticulatedHand newHand;
            ...
            while (Hands.Count > count)
            {
                ArticulatedHand lastHand = Hands[Hands.Count - 1];
