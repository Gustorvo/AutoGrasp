...
namespace SoftHand
{
    ...
    public class ArticulatedHandSettings : MonoBehaviour
    {
        ...
        private bool IsHandInitialized()
        {
            ...
            _init = _hand.Initialized; //(_hand && _hand.ArticulationBody && _settingsAsset);
            return _init;
        }
        ...
        private void ApplyPhysicalPropertiesFromAsset()
        {
            ...
            float damping = _settingsAsset.globalArticulationDriveMotorSettings.damping;
                ...
                for (int i = 0; i < _hand.Joints.Count; i++)
                {
                    ...
                    _hand.Joints[i].ArticulationBody.xDrive = SetupDrive(_hand.Joints[i].ArticulationBody.xDrive, _hand.Joints[i].Index);
                    _hand.Joints[i].ArticulationBody.yDrive = SetupDrive(_hand.Joints[i].ArticulationBody.yDrive, _hand.Joints[i].Index);
                    _hand.Joints[i].ArticulationBody.zDrive = SetupDrive(_hand.Joints[i].ArticulationBody.zDrive, _hand.Joints[i].Index);

                    ArticulationDrive SetupDrive(ArticulationDrive drive, int jointIndex)
                    {
